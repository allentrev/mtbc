...
export function btnCreate_click(event) {
    ...
    if ($w(`#box${wTarget}Choice`).rendered) { $w(`#box${wTarget}Choice`).collapse()};
    ...
    if (wTarget === "Member" || wTarget === "LiveComp" || wTarget === "RefComp"){
...
export function btnUpdate_click(event) {
    ...
    $w(`#box${wTarget}List`).collapse();
    $w(`#box${wTarget}Edit`).expand();
    if (wTarget !== "Member") { $w(`#box${wTarget}Prime`).collapse() };
    $w(`#btn${wTarget}ACreate`).hide();
    $w(`#btn${wTarget}AUpdate`).hide();
    $w(`#btn${wTarget}ADelete`).hide();
    $w(`#btn${wTarget}ASave`).show();
    ...
    if (wTarget === "Event"){
        ...
        for (let wId of wItemIds){
            ...
            if (wResult.status) {
                ...
                console.log(`public/objects/entity btnDelete_click by ${pUserId} error return from getEventBookings, err`);
    ...
    if (wTarget === "Member"){
        ...
        for (let wId of wItemIds){
            ...
            if (wResult.status) {
                ...
                console.log(`public/objects/entity btnDelete_click by ${pUserId} error return from getMTBC, err`);
                console.log(wResult.error);
            }
        }
    }
    let res = await bulkDeleteRecords("/public/objects/entity btnDelete_click", pUserId, wDataset, wItemIds);
    //let res = true;
    if (res) {
        ...
        if (wBookingsToDelete && wBookingsToDelete.length > 0){
            let res = await bulkDeleteRecords("MaintainEvent/deleteExistingBookings", pUserId, "lstBookings", wBookingsToDelete);
            if (res) {
                console.log(`/public/objects/entity btnDelete by ${pUserId} deleted ${wBookingsToDelete.length} bookings `);
            } else {
                console.log("/public/objects/entity btnDelete by ${pUserId} error from backend bulkDeleteRecords, res");
                console.log(res);
            }
        }
        if (wWixMembersToDelete && wWixMembersToDelete.length > 0){
            ...
            if (res) {
                console.log(`/public/objects/entity btnDelete by ${pUserId} deleted ${wWixMembersToDelete.length} Wix Members deleted `);
            } else {
                console.log(`/public/objects/entity btnDelete by ${pUserId} error from backend deleteWixMembers, res`);
                console.log(res);
            }
        }
        if (wMTBCMembersToDelete && wMTBCMembersToDelete.length > 0){
            let res = await bulkDeleteRecords("public/objects/entity btnDelete_click", pUserId, "lstMTBC", wMTBCMembersToDelete);
            if (res) {
                console.log(`/public/objects/entity btnDelete by ${pUserId} deleted ${wMTBCMembersToDelete.length} MTBC Members deleted `);
            } else {
                console.log(`/public/objects/entity btnDelete by ${pUserId} error from backend deleteMTBCMembers, res`);
                console.log(res);
            }
        }
        ...
        console.log(`/public/objects/entity btnDelete by ${pUserId} error from backend dulkDeleteRecords, res `);
        console.log(res);
    }
...
export async function doEntityASaveClick(pTarget) {
    ...
    try {
        ...
        let wEntity = {
            ...
            "requiredYear":  parseInt($w(`#inp${pTarget}EditRequiredYear`).value,10) || null,
            "requiredJDate": parseInt($w(`#inp${pTarget}EditRequiredJDate`).value,10) || null,
            "calKey": null,
            "status": "N",
            "summary": null
        }
        ...
        if (wResult.status) {
            ...
            switch (getMode()) { 
                ...
                case MODE.UPDATE:
                    ...
                    break;
                default:
                    console.log (`client Maintain ${pTarget} btn${pTarget}ASave invalid mode = [` + getMode() + "]");
            }
            ...
            if (wResult.savedRecord){
                console.log(`Maintain${pTarget}Event btnSave saveRecord failed, savedRecord`);
                console.log(wResult.savedRecord);
            } else {
                console.log(`Maintain${pTarget}Event btnSave saverecord failed, error`);
                console.log(wResult.error);
            }
        }
...
export function clearEditBox(pTarget) {
    ...
    switch (pTarget) {
        ...
        case "Officer":
            ...
            break;
        default:
            console.log("/public/objects/entity clearEditBox Invalid switch key", pTarget);
            break;
    }
}
/**
...
export function populateEdit(pTarget){
    ...
    switch (pTarget) {
        ...
        case "RefComp":
            ...
            $w('#rgpRefCompEditCompetitorType').value = getCompetitorType(pRec.competitorType);
            if (gMode === MODE.CREATE) {
                $w('#inpRefCompEditCompRef').enable();
            } else {
                $w('#inpRefCompEditCompRef').disable();
        ...
        case "Event":
            ...
            break;
    	default:
            console.log("public/objects/entity populateEdit 1 invalid switch target", pTarget);
			break;
    ...
    switch (wCount) {
      ...
      $w(wbtnDelete).hide();
		...
		case 1:
            ...
            } else if (pTarget === "Event" || pTarget === "Opponent" || pTarget === "Fixture"|| pTarget === "RefEvent" ||
                        pTarget === "CanEvent" || pTarget === "Officer" || pTarget === "Member") {
    			$w(wBoxPrime).expand();
            } else {
                $w(wBoxPrime).collapse();
            }
...
export function showWait(pTarget) {
    try {
        ...
        console.log(`public/objects/entity showWait Try-catch err for `, pTarget);
        console.log(err);
    }
}
/**
...
export function hideWait(pTarget) {
    try {
        ...
        console.log(`public/objects/entity hideWait Try-catch err for `, pTarget);
        console.log(err);
    }
}
