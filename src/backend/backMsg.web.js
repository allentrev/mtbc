...
export const testEmail2 = webMethod(
    ...
    async (pSubject) => {
        ...
        const msg = {
            ...
            to: [
                "Trevor1 <allentrev88@gmail.com>",
                "Trevor2 <allentrev88@outlook.com>",
            ],
        ...
        return (
            sgMail
                ...
                .catch((error) => {
                    console.error(
                        "/backend/backMsg TESTEmail catch error, err"
                    );
                    console.error(error);
...
export const sendMsgToJob = webMethod(
    ...
    async (pType, pJobKeys, pFromName, pUrgent, pTarget, pParams) => {
        ...
        for (let wJobKey of pJobKeys) {
            if (wJobKey !== null) {
                ...
                if (wResult && wResult.status) {
                    ...
                    console.warn(
                        `/backend/backMsg  sendMsgToJob couldn't find Officer, `,
                        wJobKey
                    );
                    wResult = {
                        status: false,
                        error: wResult.error || "Result undefined",
                    };
                }
            } else {
                console.warn(
                    `/backend/backMsg  sendMsgToJob Job not set `,
                    wJobKey
                );
        ...
        if (wToList && wToList.length > 0) {
            ...
            if (wResult && wResult.status) {
                console.warn(
                    `/backend/backMsg  sendMsgToJob sendMsg sent OK, msgType, JobKey `,
                    pTarget,
                    pJobKeys
                );
                ...
                console.warn(
                    `/backend/backMsg  sendMsgToJob sendMsg fail, msgType, JobKey `,
                    pTarget,
                    pJobKeys
                );
                wResult = {
...
export const sendMsg = webMethod(
    ...
    async (pType, pToList, pFromName, pUrgent, pTarget, pParams) => {
        ...
        /**
        console.log(
            "sendMsg, pType, ToList, From, Urgent, Target, Params",
            pType,
            pToList,
            pFromName,
            pUrgent,
        ...
        */
        ...
        for (let wElement of pToList) {
            if (wElement === "<All>") {
                ...
                //do email element
                [wMsgType, wContactPref] = await processEmailElement();
            } else if (wElement.includes("<")) {
                // do label
                ...
                // do send list
                [wMsgType, wContactPref] = await processMemberElement(
                    pType,
                    pUrgent,
                    pFromName,
                    wElement
        ...
        await doSendEmail(wTarget, wParams);
        await doSendSMS(wTarget, wParams);
        await doSendWhatsApp(wTarget, wParams);
...
export async function processAllList() {
    ...
    // eslint-disable-next-line no-unused-vars
    let [wEmailMembers, wSMSMembers, wWhatsAppMembers, wNoneMembers] =
        await getAllActiveMembersContact();
...
export async function processLabelList(pType, pUrgent, pFromName, pElement) {
    //console.log("processLabelList, Element",pElement);
        ...
        for (let wMatch of wMatchesText) {
            ...
            if (wResult && wResult.objects.length > 0) {
                ...
                for (let wMember of wObjectList) {
                    let [wMsgType, wContactPref] = await processMemberElement(
                        pType,
                        pUrgent,
                        pFromName,
                        wMember.memberId
...
export async function processMemberElement(
    pType,
    pUrgent,
    pFromName,
    pMemberId
) {
    //console.log("processMemberElement, Type, Urgent, FromName, MemberID",
    ...
    if (wResult && wResult.status) {
        ...
        [wMsgType, wContactPref] = await validateMsgTypeAndContactPref(
            pType,
            wResult.member
        );
        ...
        console.warn(
            `/backend/backMsg  processMemberElement couldn't find member, `,
            pMemberId
        );
    }
...
export async function processEmailElement() {
    // should never get here from Maintain Notice as S type converts to a list of memberIds
    //console.log("processEmailElement, Type, Urgent, FromName",
    //  pType, pUrgent, pFromName);
...
export async function doMsgSortingOffice(pMsgType, pContactPref) {
    //console.log("doMsgSortingOffice, MsgType, ContactPref",
    ...
    let wToWhatsAppEntry = {
        name: gControl.toName,
        mobile: gControl.mobilePhone,
    };
    ...
    switch (pMsgType) {
        ...
        case "U":
            switch (pContactPref) {
                ...
                default:
                    console.warn(
                        `/backend/backMsg  sendMsg fail, invalid contactpref`,
                        pContactPref
                    );
                    break;
            } // contactpref switch
        ...
        default:
            console.warn(
                `/backend/backMsg  sendMsg fail, invalid type [${pMsgType}]`
            );
            break;
    } // type switch
    return true;
...
export async function validateMsgTypeAndContactPref(pType, pMember) {
    //console.log("validateMsgTypeAndContactPref, Type, Member",
    //  pType, pMember);
...
async function doSendEmail(pTarget, pParams) {
    //console.log("doSendEmail, pTarget, Params, length",
    ...
    if (gToEmailList && gToEmailList.length > 0) {
        ...
        if (wResult && wResult.status) {
            ...
            console.warn(
                `/backend/backMsg  sendMsg sendEmail fail, msgType, JobKey `,
                pTarget
            );
            wEmailResult = {
                status: false,
                error: wEmailResult.error || "Result undefined",
            };
        }
    } else {
...
async function doSendSMS(pTarget, pParams) {
    //console.log("doSendSMS, pTarget, Params, length",
    ...
    if (gToSMSList && gToSMSList.length > 0) {
        ...
        if (wResult && wResult.status) {
            ...
            console.warn(
                `/backend/backMsg  sendMsg sendSMS fail, msgType, JobKey `,
                pTarget
            );
            wSMSResult = {
                status: false,
                error: wSMSResult.error || "Result undefined",
            };
        }
    } else {
...
async function doSendWhatsApp(pTarget, pParams) {
    //console.log("doSendWhatsApp, pTarget, Params, length",
    ...
    if (gToWhatsAppList && gToWhatsAppList.length > 0) {
        ...
        if (wResult && wResult.status) {
            ...
            console.warn(
                `/backend/backMsg  sendMsg sendWhatsApp fail, msgType, JobKey `,
                pTarget
            );
            wWhatsAppResult = {
                status: false,
                error: wWhatsAppResult.error || "Result undefined",
            };
        }
    } else {
...
async function sendEmail(pTarget, pControl, pParams) {
    //console.log("sendEmail, pTarget, Control, Params",
    ...
    if (wResult1 && wResult1.status) {
        ...
        if (wResult2 && wResult2.status) {
            ...
            console.warn(
                `/backend/backMsg  sendEmail transmitEmail failed, error`
            );
...
async function sendSMS(pTarget, pControl, pParams) {
    //console.log("sendSMS, pTarget, Control, Params",
    //  pTarget, pControl, pParams);
...
async function sendWhatsApp(pTarget, pControl, pParams) {
    ...
    if (wResult1 && wResult1.status) {
        ...
        // eslint-disable-next-line no-unused-vars
        const wMessage = wResult1.sms.body;
        const wResult2 = await transmitWhatsApp(
            wTo,
            wSubject + "\n" + wMessage
        );
        if (wResult2 && wResult2.status) {
            ...
            console.warn(
                `/backend/backMsg  sendWhatsApp transmitWhatsAppSMS failed, error`
            );
            console.log(wResult.error || "Result undefined");
            wResult = {
                status: false,
        ...
        console.warn(
            `/backend/backMsg  sendWhatsApp createWhatsAppSMS failed, error`
        );
        console.log(wResult.error || "Result undefined");
        wResult = { status: false, error: wResult.error || "Result undefined" };
    }
...
async function createEmail(pTarget, pControl, pParams) {
    //console.log("createEmail, pTarget, Control, Params",
    //    pTarget, pControl, pParams );
...
async function createSMS(pTarget, pControl, pParams) {
    //console.log("createSMS, pTarget, Control, Params",
    //  pTarget, pControl, pParams);
    ...
    switch (pTarget) {
        ...
        default:
            console.warn(
                `/backend/backMsg  creatSMS invalid Email Target`,
                pTarget
            );
            wStatus = false;
...
async function Blank1_Email(pParameters) {
    //console.log("Blank1_Email, Parameters",pParameters);
...
async function Blank1_SMS(pParameters) {
    //console.log("Blank1_SMS, Parameters", pParameters);
...
export async function transmitEmail(pToList, pSubject, pMsg) {
    //console.log("transmitEmail, pToList, Subject, Msg", pToList, pSubject, pMsg );
    ...
    return (
        sgMail
            ...
            .catch((error) => {
                console.error(
                    "/backend/backMsg transmitEmail catch error, err, msg"
                );
...
export async function transmitSMS(pTo, pMsg) {
    //console.log("transmitSMS, pTo, Msg", pTo, pMsg);
...
export async function transmitWhatsApp(pTo, pMsg) {
    //console.log("transmitWhatsApp, pTo, Msg", pTo, pMsg);
