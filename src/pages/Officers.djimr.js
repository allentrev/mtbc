...
$w.onReady(async function () {
    try {
        ...
        if (isLoggedIn) {
            $w("#txtIntro").text =
                ` As a member, you can double click on a person's name to load that person's contact details`;
        } else {
            $w("#txtIntro").text =
                `As a visitor to this site, simply select from the dropdown the committee that you want to see the members of.`;
        }
        await loadListData();
        ...
        $w("#btnContactCardClose").onClick((event) =>
            doBtnContactCardCloseClick(event)
        );
        $w("#btnCopyToClipboard").onClick((event) =>
            doBtnCopyToClipboardClick()
        );
...
async function loadDropDownData() {
    ...
    if (wResult.status) {
        ...
        console.log(
            "/page Officers loadDropDownData error reading committee list"
        );
    }
}
...
async function loadRptJobs($item, itemData) {
    ...
    $item("#lblListMobilePhone").text = itemData.mobilePhone || "no phone #";
    $item("#lblListHomePhone").text = itemData.homePhone || "no phone #";
}
...
async function loadRptTeamCaptains($item, itemData) {
    ...
    $item("#lblListTCMobilePhone").text = itemData.mobilePhone || "no phone #";
    $item("#lblListTCHomePhone").text = itemData.homePhone || "no phone #";
}
...
function doLblListMemberClick(event) {
    ...
    if (wControlName === "lblListName") {
        ...
        wMobilephone = wControl(`#lblListMobilePhone`).text || "no phone #";
        wHomePhone = wControl(`#lblListHomePhone`).text || "no phone #";
    } else {
        ...
        wMobilephone = wControl(`#lblListTCMobilePhone`).text || "no phone #";
        wHomePhone = wControl(`#lblListTCHomePhone`).text || "no phone #";
    }
    if (wHolderId === "") {
        return;
    }
    ...
    if (wAllowShare === "N") {
        ...
        $w("#lblMobilePhone").text =
            hyphenatePhoneNumber(wMobilephone) || "No moile#";
        $w("#lblHomePhone").text =
            hyphenatePhoneNumber(wHomePhone) || "No home#";
    }
}
...
export function doBtnCopyToClipboardClick() {
    wixWindow
        .copyToClipboard($w("#lblEmail").text)
        .then(() => {
    ...
    for (let wJob of pJobs) {
        ...
        if (wJob.holderId) {
            ...
            if (wResult.status) {
                ...
                wEntry.mobilePhone = wMember.mobilePhone || "no phone #";
                wEntry.homePhone = wMember.homePhone || "no phone #";
            } else {
                console.log(
                    "/page Officers loadJobs couldnt find member, id",
                    wJob.holderId
                );
                return;
            }
    ...
    for (let wTeam of pTeams) {
        ...
        if (
            ...
            if (wTeam.gender === "M") {
                ...
                wPosition =
                    (await showLeagueName(wTeam.leagueKey, wTeam.division)) +
                    " " +
                    wTeamCode;
            } else {
                let wLeagueName = await showLeagueName(
                    wTeam.leagueKey,
                    wTeam.division
                );
        ...
        if (wTeam.managerId) {
            ...
            if (wResult.status) {
                ...
                console.log(
                    "/page Officers loadJobs couldnt find captain, id",
                    wTeam.managerId
                );
                wName = "Not found";
                wEntry.holderId = "";
...
function showError(pErr) {
    let wMsg = [
        "There are no inter-club teams defined",
