...
$w('#textBox1').value =`Steve Wheeler\n\tTrevor Allen`;
$w('#text51').html = `<span style="font-size: 14px">Steve Wheeler & Trevor Allen
					<br><span style="margin-left: 20px">Tony Stuart, Richard Smith</span>`;
*/
...
export async function loadData() {
	...
	loadOpenSinglesTable("OP01", "Ladder A")
	.then( (wDataA) => {
		transform(wDataA)
	...
	loadOpenSinglesTable("OP01", "Ladder B")
	.then( (wDataB) => {
		transform(wDataB);
	...
	loadOpenSinglesTable("OP01", "Ladder Final")
	.then( (wDataF) => {
		transform(wDataF)
...
async function loadRepeaterR2B($item, itemData, index) {
	...
	async function loadRepeaterFinal($item, itemData, index) {
		...
		if (index === 0 ) {
			...
			$item("#txtFinalPlayer").text = convertNulls(itemData.name);
