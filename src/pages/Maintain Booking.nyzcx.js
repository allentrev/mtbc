    ...
    try {
        ...
        */
        ...
        if (isLoggedIn) {
            ...
            console.log("/page/MaintainBooking onReady  Roles = <" + wRoles + ">", loggedInMember.name, loggedInMember.lstId);
        } else {
...
//-----------------------------------------------------Main -----------------------------
        ...
        $w('#btnBookingADelete').onClick((event) => btnDelete_click(loggedInMember.lstId, event));
        ...
        $w('#btnBookingAToSpecial').onClick((event) => doBtnBookingAToSpecialClick(event));
        $w('#btnBookingAToEvent').onClick((event) => doBtnBookingAToEventClick());
        
        //$w('#btnBookingAPrime').onClick((event) => btnBookingAPrime_click(event));
        $w('#chkBookingListSelect').onClick((event) => chkSelect_click(event));
        $w('#chkBookingListSelectAll').onClick((event) => chkSelectAll_click(event));
        $w('#btnBookingListTop').onClick((event) => btnTop_click(event));
        $w('#pgnBookingList').onClick((event) => doPgnListClick(event));
        $w('#inpBookingListNoPerPage').onChange((event) => doInpListNoPerPageChange(event));
        $w('#btnBookingEditPlayerAAdd').onClick((event) => doBtnBookingPlayerAdd("EditPlayerA"));
        $w('#btnBookingEditPlayerAClear').onClick((event) => doBtnBookingPlayerClear("EditPlayerA"));
        $w('#btnBookingEditPlayerBAdd').onClick((event) => doBtnBookingPlayerAdd("EditPlayerB"));
        $w('#btnBookingEditPlayerBClear').onClick((event) => doBtnBookingPlayerClear("EditPlayerB"));
        $w('#btnBookingEditBookerAdd').onClick((event) => doBtnBookingPlayerAdd("EditBooker"));
        $w('#btnBookingEditBookerClear').onClick((event) => doBtnBookingPlayerClear("EditBooker"));
        $w('#btnBookingChoiceBookerAdd').onClick((event) => doBtnBookingPlayerAdd("ChoiceBooker"));
        $w('#btnBookingChoiceBookerClear').onClick((event) => doBtnBookingPlayerClear("ChoiceBooker"));
//##
        $w('#rgpBookingChoice').onChange((event) => doBookingChoiceChange(event));
        $w('#btnBookingChoiceGo').onClick((event) => doBookingChoiceGoClick(event));
        $w('#rgpBookingChoiceDate').onChange((event) => doBookingChoiceDateChange(event));

//      Specials Section
        $w('#btnSpecialACreate').onClick((event) => doBtnSpecialCreateClick());
        $w('#btnSpecialACancel').onClick((event) => doBtnSpecialCancelClick());
        $w('#btnSpecialAToBooking').onClick((event) => doBtnSpecialAToBookingClick(event));
        $w('#btnSpecialAToRecurring').onClick((event) => doBtnSpecialAToRecurringClick(event));
        $w('#btnSpecialAToAllDay').onClick((event) => doBtnSpecialAToAllDayClick(event));
        $w('#drpSpecialUse').onChange( (event) => drpSpecialUse_change(event));
        $w('#inpSpecialUse').onChange( (event) => inpSpecialUse_change(event));
        $w('#btnSpecialDailyReport').onClick( (event) => btnSpecialDailyReport_click(event));
        $w('#btnSpecialMoveAllDay').onClick((event) => doBtnSpecialMoveAllDayClick());
//      Bookings Rink Slot Section
        $w('#inpBookingEditStartRink').onChange( (event) => inpStartRink_change("BookingEdit", event));
        $w('#inpBookingEditNoRinks').onChange( (event) => inpNoRinks_change("BookingEdit", event));
        $w('#dpkBookingEditFromDate').onChange( (event) => dpkFromDate_change("BookingEdit", event));
        $w('#tpkBookingEditStartTime').onChange( (event) => tpkStartTime_change("BookingEdit", event));
        $w('#tpkBookingEditDuration').onChange( (event) => tpkDuration_change("BookingEdit", event));
//      Specials Rink Slot Section
        $w('#inpSpecialRecurringStartRink').onChange( (event) => inpStartRink_change("SpecialRecurring", event));
        $w('#inpSpecialRecurringNoRinks').onChange( (event) => inpNoRinks_change("SpecialRecurring", event));
        $w('#rgpSpecialRecurringDayOfWeek').onChange( (event) => rgpSpecialRecurringDayOfWeek_change(event));
        $w('#dpkSpecialRecurringFromDate').onChange( (event) => dpkFromDate_change("SpecialRecurring", event));
        $w('#tpkSpecialRecurringStartTime').onChange( (event) => tpkStartTime_change("SpecialRecurring", event));
        $w('#tpkSpecialRecurringDuration').onChange( (event) => tpkDuration_change("SpecialRecurring", event));
//      Specials Move All Day Section
        $w('#dpkSpecialMoveFromDate').onChange( (event) => doDpkSpecialMoveDateChange("F", event));
        $w('#dpkSpecialMoveToDate').onChange( (event) => doDpkSpecialMoveDateChange("T", event));
...
async function loadRptBookingList($item, itemData, index) {
    ...
    if (index === 0) {
        ...
        let wDate = (itemData.dateRequired === null || itemData.dateRequired === undefined) ? "" : formatDateString(itemData.dateRequired, "Short");
...
//------------------------------------------validations--------------------------
	...
	wBooking.resourceKey =  await getResourceKey(wYear, wJDate, wStartRink, wFromSlot);
    ...
    switch (getMode()) {
        case MODE.CREATE:
            wResult = await processRecord(wStartDateTime, wTime.substring(0,5), wYear, wJDate, wStartRink, wNoRinks, wSlotRange, wFromSlot, wToSlot);
            break;
        case MODE.UPDATE:
            ...
            if (gSlotRinkChange) {
                ...
                if (wResult.status){
                    ...
                    if (wResult.savedRecord) {
                        console.log("/page/MaintainBooking btnBookingASave, save failed, savedRecord", wResult.savedRecord);
                    } else {
                        console.log("/page/MaintainBooking btnBookingASave, save failed, error", wResult.error);
                    }
                }
            }
            break;
        default:
            console.log("/page/MaintainBooking btnBookingASave Booking Save mode = ", getMode());
        }
...
export function doBookingChoiceChange(event) {
    ...
    (wValue === "A") ? $w('#boxBookingChoiceBooker').collapse() : $w('#boxBookingChoiceBooker').expand();  
}
...
export function doBookingChoiceDateChange(event){
    ...
    (wValue === "O") ? $w('#boxBookingChoiceDateOn').expand() : $w('#boxBookingChoiceDateOn').collapse();  
    (wValue === "F") ? $w('#boxBookingChoiceDateFrom').expand() : $w('#boxBookingChoiceDateFrom').collapse();  
    (wValue === "R") ? $w('#boxBookingChoiceDateRange').expand() : $w('#boxBookingChoiceDateRange').collapse();  
}
...
//----------------------Linked Bookings-------------------
    ...
    if (wSelectedRecord.hasChildren === "Y") {
        ...
        if (wResult.status) {
            ...
            console.log("/page/MaintainBooking populateEventEdit getEventBookings Failed, error");
...
async function processRecord(pDate, pTime, pYear, pJDate, pStartRink, pNoRinks, pSlotRange, pFromSlot, pToSlot){
    ...
    try {
        ...
        let wBookingsList = await addBookings(wBookerId, wManualEvent, pStartRink, pNoRinks, pSlotRange, pFromSlot, pToSlot, wParams);
...
export async function deleteBookingList(pList){
    ...
    if (pList.length > 0) {
        let res = await bulkDeleteRecords("MaintainBooking/deleteBookingsList", loggedInMember.lstId, "lstBookings", pList);
        await deleteGlobalDataStore(pList, "Booking");
        if (res) { await showError("Booking", 3, 13) };
    } else {
        console.log("/page/MaintainBooking deleteBookingList called with nothing in list");
    }
}
...
export function rgpSpecialRecurringDayOfWeek_change(event) {
    ...
    try {
        ...
        console.log(`/page/MaintainBooking rgpSpecialRecurringDayOfWeek_change Try-catch error`);
        console.log(err);
    }
}
...
export function tpkDuration_change(pTarget, event) {
    try {
        ...
        let wNewTime = addDurationToTime($w(`#tpk${pTarget}StartTime`).value, wDuration);
        $w(`#tpk${pTarget}EndTime`).value = wNewTime;
        gSlotRinkChange = true;
        doCalcTimeChange(pTarget);
    }
    catch (err){
        console.log(`/page/MaintainBooking tpkDuration_change ${pTarget} Try-catch error`);
        console.log(err);
    }
}
...
export function tpkStartTime_change(pTarget, event) {
    try {
        ...
        let wNewTime = addDurationToTime(wTime, $w(`#tpk${pTarget}Duration`).value);
        $w(`#tpk${pTarget}EndTime`).value = wNewTime;
        gSlotRinkChange = true;
        doCalcTimeChange(pTarget);
    }
    catch (err){
        console.log(`/page/MaintainBooking tpkStartTime_change ${pTarget} Try-catch error`);
        console.log(err);
    }
}
...
function doCalcTimeChange(pTarget){
    try {
        $w(`#inp${pTarget}FromSlot`).value = String(getStartSlot($w(`#tpk${pTarget}StartTime`).value)+1);
        $w(`#inp${pTarget}ToSlot`).value = String(getEndSlot($w(`#tpk${pTarget}EndTime`).value)+1);
    }
    catch (err){
        console.log(`/pageMaintainBooking doCalcTimeChange ${pTarget} Try-catch error`);
        console.log(err);
    }
}
...
export async  function dpkFromDate_change(pTarget, event) {
    try {
        ...
        console.log(`/page/MaintainBooking dpkFromDate_change ${pTarget} Try-catch error`);
        console.log(err);
    }
}
...
export async function doCalcRinksSlots(pTarget) {
    try {
        ...
        let [wRinksFree, wNoSlots, wStartRink, wSlotRange, wFromSlot, wToSlot] = await getRinksAndSlots(wRequiredDate, wStartTime, wDuration);
        ...
        if (wRinksFree === 0) {
            ...
            $w(`#inp${pTarget}NoRinks`).value = String(wRinksFree - wStartRink + 1);
        ...
        console.log(`/page/MaintainBooking doCalcRinksSlots ${pTarget} Try-catch error`);
        console.log(err);
    }
}
...
export function inpNoRinks_change(pTarget, event) {
    try {
        ...
        console.log(`/page/MaintainBooking inpNoRinks_change ${pTarget} Try-catch error`);
        console.log(err);
    }
}
...
export function inpStartRink_change(pTarget, event) {
    try {
        ...
        console.log(`/page/MaintainBooking inpStartRink_change_change ${pTarget} Try-catch error`);
        console.log(err);
    }
}
...
function rgpSpecialRecurringDayOfWeekChange(pDay){
    try {
        ...
        console.log(`/page/MaintainBooking rgpSpecialRecurringDayOfWeekChange Try-catch error`);
        console.log(err);
    }
}
...
export async function doBtnSpecialCreateClick () {
    ...
    if (gSpecialMode === MODE.PRIME){
	    if (!$w('#inpSpecialRecurringSlotRange').valid) { showLocalError(2); ($w('#inpSpecialRecurringSlotRange').focus()); return}; 
	    if (!$w('#inpSpecialRecurringNoRinks').valid) { showLocalError(2); ($w('#inpSpecialRecurringNoRinks').focus()); return}; 
	    if (!$w('#inpSpecialRecurringStartRink').valid) { showLocalError(2); ($w('#inpSpecialRecurringStartRink').focus()); return}; 
	    if (!$w('#inpSpecialRecurringNoOfPlayers').valid) { showLocalError(2); ($w('#inpSpecialRecurringNoOfPlayers').focus()); return}; 
    } else {
    ...
    let wResult;
	if (gSpecialMode === MODE.CREATE) { wResult = await performAllDay()}
	if (gSpecialMode === MODE.PRIME) { wResult = await performRecurring()}
	if (gSpecialMode === MODE.MOVE) { wResult = await performMoveAllDay()}
    if (wResult.status){
        let wNewBookings = wResult.bookings;
        for (let wBooking of wNewBookings){
            updateGlobalDataStore(wBooking,"Booking");
...
export async function doBtnSpecialCancelClick () {
    console.log("/page/MaintainBooking doBtnSPecialCancel Cancel ", gSpecialMode);
}
		...
		for (let x = 0; x < 7; x++) {
			...
			if (x === 0) {
				...
				if (wCell.status !== "B") {
					...
					let wTemp = w_book_out.filter( item => item.slotId === y && item.rink === x);
...
function getNamesString(pCell){
	...
	if (wAId === null || wAId === "undefined" || wAId === TEMPORARY_HOLDER) {
		if (wBId === null || wBId === "undefined" || wBId === TEMPORARY_HOLDER) {
			return "";
		} else { 
...
async function getTeamNames(pRec, pCompRef, pMatchKey){
	...
	const wCompetitors = wAllCompetitors.filter (item => item.stage === wStage)
										.filter (item => item.div === wDiv);
	//console.log("filtered", wStage, wDiv);
	//console.log(wCompetitors);
	const wCompetitorA = wCompetitors.filter ( item => item.skipId === pRec.playerAId);
	const wCompetitorB = wCompetitors.filter ( item => item.skipId === pRec.playerBId);
...
function checkBookingsForEvents(pBookings){
    if (pBookings && pBookings.length > 0){
        let wEventBookings = pBookings.filter( item => (item.eventId !== undefined && item.eventId !== null && item.eventId !== ""));
        let wMasterBookings = wEventBookings.filter( item => item.parentId === "");
...
export async function performAllDay(){
    ...
    let [wRinksFree, wNoSlots, wStartRink, wSlotRange, wFromSlot, wToSlot] = await getRinksAndSlots(wDate, "10:00", 10);

	let wBookingsList = await addBookings(wBookerId, wManualEvent, 1, wRinksFree, wSlotRange, wFromSlot, wToSlot, wParams);
    let wResult = await processEventBookings("", wBookingsList);
    ...
    if (wResult.status){
        ...
        console.log("/page/MaintainBooking performAllDay error writing bookings");
		return wResult;
    }
}
...
export async function performRecurring(){
	...
	while (wJDate <= wTo && wJDate < 367){ 
        ...
        let [wRinksFree, wStart] = await getNoFreeRinks(gYear, wJDate, wSlotRange, wFromSlot);

        if (wRinksFree === 0) {
            console.log("/page/MaintainBooking performRecurring No rinks available to book for this date", wDateRequired);
            showLocalError(6);
            return;
        } else if (wRinksFree < wNoRinks) {
            wBookingsList = await addBookings(wBookerId, wManualEvent, wStartRink, wRinksFree, wSlotRange, wFromSlot, wToSlot, wParams);
            console.log(`/page/MaintainBooking performRecurring  Only ${wRinksFree} bookings out of ${wNoRinks} can be recorded for date`, wDateRequired.toLocaleDateString());
        } else {
            wBookingsList = await addBookings(wBookerId, wManualEvent, wStartRink, wNoRinks, wSlotRange, wFromSlot, wToSlot, wParams);
            console.log(`/page/MaintainBooking performRecurring  Made all (${wNoRinks}) bookings recorded for date`,  wDateRequired.toLocaleDateString());
        }
        let wResult = await processEventBookings("", wBookingsList);
        //let wStatus = true;
        if (wResult.status){
            ...
            console.log("/page/MaintainBooking performRecurring  error writing bookings, err");
            console.log(wResult.error);
        }
...
export async function performMoveAllDay(){
    ...
    if (parseInt($w('#lblSpecialMoveNoFrom').text,10) === 0 ){
		...
		return {"status": false, "result": null, "error": "From Date validation error"};
    }
    if (parseInt($w('#lblSpecialMoveNoTo').text,10) > 0 ){
        showLocalError(9);
        $w('#dpkSpecialMoveToDate').focus();
		return {"status": false, "result": null, "error": "To Date validation error"};
    }
    ...
    if (wResult.status){
        ...
        if (wResult2.status) {
            ...
            console.log("/page/MaintainBooking performMoveAllDay eventUpdate fail, err");
            console.log(wResult.error);
            return {"status": false, "bookings": [], "error": wResult2.error};
        }
    } else {
        console.log("/page/MaintainBooking performMoveAllDay writeNewBooking fail, err");
        console.log(wResult.error);
...
async function updateEvents(pToDate) {
    if ($w('#tblSpecialMoveEvents').collapsed) { return {"status": true, "events": [], "error": ""} }
    ...
    for (let wEvent of wEventList){
        ...
        if (!wResult.status){
            ...
            console.log(wResult.error);
            return {"status": false, "event": wResult.event , "error": wResult.error};
        }
...
async function writeNewBookings(pToDate){
    ...
    */
    if (gBookingsToMove && gBookingsToMove.length == 0) { console.log("/page/MaintainEvent performRecurring writeNewBookings Nowt")};
    ...
    for (let wOldBooking of wSortedBookings){
        ...
        let wResourceKey = String(wToYear) + String(wToJDate).padStart(3,"0") + "S" + String(wOldBooking.slotId).padStart(2,"0")
                         + "R" + String(wOldBooking.rink).padStart(2,"0");
        ...
        let wDateRequired = new Date(wToYear, wToMonth, wToDate, wHours, wMins,0);
        ...
        if (wResult.status) {
            ...
            if (wOldBooking.hasChildren === "Y" ) { wLastId = wSavedRecord._id };
            wNewBookings.push(wSavedRecord);
        } else {
            console.log("/page/MaintainBooking writeNewBooking fail, resourceKey = ", wResourceKey, " err" );
            console.log(wResult.error);
            wStatus = false;
        }      
    }
    if (wStatus){
        ...
        return { "status": false, "bookings": [], "error": "failed to write new bookings"} 
    }
}   
