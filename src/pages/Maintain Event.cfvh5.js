    ...
    try {
        ...
        */
        ...
        if (isLoggedIn) {
            ...
            console.log(
                "/page/MaintainEvent onReady  Roles = <" + wRoles + ">",
                loggedInMember.name,
        ...
        $w("#inpEventListNoPerPage").onChange((event) =>
            doInpListNoPerPageChange(event)
        );

        $w("#drpEventEditRinks").onChange((event) =>
            doDrpEventEditRinksChange(event)
        );
        $w("#drpEventEditHomeAway").onChange((event) =>
            doDrpEventEditRinksChange(event)
        );
        $w("#tpkEventEditStartTime").onChange((event) =>
            doDrpEventEditRinksChange(event)
        );
        $w("#tpkEventEditDuration").onChange((event) =>
            doDrpEventEditRinksChange(event)
        );
        $w("#dpkEventEditStartDate").onChange((event) =>
            doDrpEventEditRinksChange(event)
        );
...
function loadRptEventList($item, itemData, index) {
    if (index === 0) {
        ...
        let wDate =
            itemData.startDate === null || itemData.startDate === undefined ?
                ""
            :   formatDateString(itemData.startDate, "Short");
        ...
        wSubject =
            itemData.status === EVENT.CANCELLED ?
                wSubject + " (Cancelled)"
            :   wSubject;
        //if (wUploaded){
...
function refreshRptEvents(pEvent) {
    ...
    if (wRptData.some((item) => item._id === pEvent._id)) {
        $w("#rptEventList").forItems([pEvent._id], ($item) => {
            let wDate =
                pEvent.startDate ?
                    formatDateString(pEvent.startDate, "Short")
                :   "";
...
async function populateEventEditDropDowns() {
    let wLeagues = [{ label: "None", value: "X" }];
    let wTeams = [{ label: "None", value: "X" }];
...
export async function btnEventASave_click(event) {
    ...
    wEvent.calKey = await getCalKey(
        $w("#drpEventEditEventType").value,
        $w("#drpEventEditLeague").value
    );
    ...
    switch (getMode()) {
            ...
            if (gSlotRinkChange) {
                ...
                if (wResult.status) {
                    ...
                    if (wResult.savedRecord) {
                        console.log(
                            "/page/MaintainEvent btnEventASave, save failed, savedRecord",
                            wResult.savedRecord
                        );
                    } else {
                        console.log(
                            "/page/MaintainEvent btnEventASave, save failed, error",
                            wResult.error
                        );
                    }
                }
        ...
        case MODE.CANCELLATION:
            ...
            if (wResult.status) {
                ...
                console.log(
                    "/page/MaintainEvent btnEventASave updateEvent Status failed"
                );
                showError("Event", 20);
            }
            break;
        default:
            console.log(
                "/page/MaintainEvent btnEventASave Event Save mode = ",
                getMode()
            );
    }
    resetSection("Event");
...
export async function populateEventEdit() {
    ...
    $w("#btnEventASave").label =
        getMode() === MODE.CANCELLATION ? "Confirm" : "Save";
    ...
    if (wSelectedRecord.homeAway === "H") {
        ...
        if (wResult.status) {
            ...
            if (wBookings.length > 0) {
                ...
                let [wNoFreeRinks, wStartRink] = await getNoFreeRinks(
                    gYear,
                    wFirstRec.requiredJDate,
                    wFirstRec.rangeId,
                    wFirstRec.slot
                );
            ...
            console.log(
                "/page/MaintainEvent populateEventEdit getEventBookings Failed, error"
            );
        ...
        case "Event":
            ...
            if (wResult.status) {
                ...
                if (wEventBookings) {
                    ...
                    */
                        let wResult = await processEventBookings(
                            wSavedRec._id,
                            wEventBookings
                        );
                    }
                ...
                if (wResult.savedRecord) {
                    console.log(
                        "/page/MaintainEvent ProcessRecord, save failed, savedRecord",
                        wResult.savedRecord
                    );
                } else {
                    console.log(
                        "/page/MaintainEvent ProcessRecord, save failed, error",
                        wResult.error
                    );
                }
            }
...
async function processEventRecord(pItem) {
    ...
    let wRinks =
        isNaN(parseInt(pItem.rinks, 10)) ? 0 : parseInt(pItem.rinks, 10);
...
async function addEventBookings(pEvent) {
    ...
    if (wRinksFree === 0) {
        console.log(
            "/page/MaintainEvent addEventBookings No rinks available to book, event.Id, event.subject  ",
            pEvent._id,
            pEvent.subject
        );
    } else if (wRinksFree < parseInt(pEvent.rinks, 10)) {
        wEventBookings = await addBookings(
            wBookerId,
            pEvent,
            wStartRink,
            wRinksFree,
        ...
        console.log(
            `client MaintainEvent addEventBookings Only ${wRinksFree} bookings our of ${pEvent.rinks} can be recorded for`,
            pEvent.subject
        );
    } else {
        wEventBookings = await addBookings(
        ...
        console.log(
            `client MaintainEvent addEventBookings Made all (${pEvent.rinks}) bookings recorded for`,
            pEvent.subject
        );
    }
...
export async function deleteExistingBookings() {
    if ($w("#boxEventEditBookings").collapsed) {
        return;
    }
