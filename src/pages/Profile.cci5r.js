    ...
    try {
		...
		*/
        ...
        if (isLoggedIn) {
            ...
            console.log(
                "/membersArea/profile onready Roles = <" + wRoles + ">",
                loggedInMember.name,
            ...
            if (wResult && wResult.status) {
                ...
                console.log(
                    "/membersArea/profile onReady/ couldnt find member, id",
                    loggedInMember.lstId
                );
            }
        } else {
        ...
        $w("#btnMemberEditMoreDisplay").onClick((event) =>
            btnMemberEditMoreDIsplayClick()
        );
        $w("#btnMemberASave").onClick((event) => btnMemberASave_click(event));
        $w("#btnEditProfile").onClick(() => btnEditProfile_click());
        $w("#btnMemberCancel").onClick(() => btnMemberCancel_click());
...
function populateEdit(pTarget) {
    ...
    switch (pTarget) {
        case "Member":
            ...
            if (
                /* already registered with Wix, then cannoot change login email */ wSelected.wixId
            ) {
        ...
        default:
            console.log(
                "/membersArea/profile populateEdit Invalid switch key",
                pTarget
            );
            break;
    }
...
export async function btnMemberASave_click(event) {
    try {
        ...
        let wMember = {
            ...
            mobilePhone: await formPhoneString(
                "mobile",
        ...
        if (wResult && wResult.status) {
            ...
            console.log(
                "/membersArea/profile  btnMemberASave_click saveRecord OK for ",
                gMember._id
            );
            showError("Member", 7);
            //
            ...
            if (wChangeList !== "") {
                ...
                if (wResult && wResult.status) {
                    console.log(
                        "/membersArea/profile  btnMemberASave_click saveRecord sendMsgToJob OK for ",
                        gMember._id
                    );
                } else {
                    console.log(
                        "/membersArea/profile  btnMemberASave_click saverecord sendMsgToJob failed, error"
                    );
            ...
            if (wResult && wResult.savedRecord) {
                console.log(
                    "/membersArea/profile  btnMemberASave_click saveRecord failed, savedRecord, error"
                );
                console.log(wResult.savedRecord);
                console.log(wResult.error);
            } else if (wResult) {
                console.log(
                    "/membersArea/profile  btnMemberASave_click saverecord failed, error"
                );
                console.log(wResult.error);
            } else {
                console.log(
                    "/membersArea/profile  btnMemberASave_click wResult undefined"
                );
                console.log(wResult.error);
            }
        }
        ...
        console.log(
            "/membersArea/profile  btnMemberASave_click Try-catch, err"
        );
        console.log(err);
        //if (!gTest) { wixLocation.to("/syserror") };
    }
...
function compareNewToOld(pMember) {
    ...
    if (gOldMember.firstName.trim() !== pMember.firstName.trim()) {
        wChangeList =
            wChangeList +
            `Firstname changed from ${gOldMember.firstName} to ${pMember.firstName}<br>`;
    }
    if (gOldMember.surname.trim() !== pMember.surname.trim()) {
        wChangeList =
            wChangeList +
            `Surname changed from ${gOldMember.surname} to ${pMember.surname}<br>`;
    }
    if (gOldMember.gender !== pMember.gender) {
        wChangeList =
            wChangeList +
            `Gender changed from ${gOldMember.gender} to ${pMember.gender}<br>`;
    }
    if (gOldMember.contactEmail.trim() !== pMember.contactEmail.trim()) {
        wChangeList =
            wChangeList +
            `Contact email changed from ${gOldMember.contactEmail} to ${pMember.contactEmail}<br>`;
    }
    if (gOldMember.mobilePhone.trim() !== pMember.mobilePhone.trim()) {
        wChangeList =
            wChangeList +
            `Mobile phone changed from ${gOldMember.mobilePhone} to ${pMember.mobilePhone}<br>`;
    }
    if (gOldMember.homePhone.trim() !== pMember.homePhone.trim()) {
        wChangeList =
            wChangeList +
            `Home phone changed from ${gOldMember.homePhone} to ${pMember.homePhone}<br>`;
    }
    if (gOldMember.addrLine1.trim() !== pMember.addrLine1.trim()) {
        wChangeList =
            wChangeList +
            `Address Line 1 changed from ${gOldMember.addrLine1} to ${pMember.addrLine1}<br>`;
    }
    if (gOldMember.addrLine2.trim() !== pMember.addrLine2.trim()) {
        wChangeList =
            wChangeList +
            `Address line 2 changed from ${gOldMember.addrLine2} to ${pMember.addrLine2}<br>`;
    }
    if (gOldMember.town.trim() !== pMember.town.trim()) {
        wChangeList =
            wChangeList +
            `Town changed from ${gOldMember.town} to ${pMember.town}<br>`;
    }
    if (gOldMember.postCode.trim() !== pMember.postCode.trim()) {
        wChangeList =
            wChangeList +
            `Postcode changed from ${gOldMember.postCode} to ${pMember.postCode}<br>`;
    }
    return wChangeList;
}
    ...
    if ($w("#upbMemberEditPhoto").value.length > 0) {
        ...
        $w("#upbMemberEditPhoto")
            ...
            .catch((uploadError) => {
                ...
                console.log(
                    "/membersArea/profile  File upload error: " +
...
export function showWait(pTarget) {
    try {
        ...
        console.log(
            `/membersArea/profile showWait Try-catch err for `,
            pTarget
        );
        console.log(err);
    }
...
export function hideWait(pTarget) {
    try {
        ...
        console.log(
            `/membersArea/profile hideWait Try-catch err for `,
            pTarget
        );
        console.log(err);
    }
